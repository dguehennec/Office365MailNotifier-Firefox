...
Components.utils.import("resource://office365_mail_notifier/controller/service.jsm");

XPCOMUtils.defineLazyServiceGetter(Services, "res", "@mozilla.org/network/protocol;1?name=resource", "nsIResProtocolHandler");
XPCOMUtils.defineLazyServiceGetter(Services, "appstartup", "@mozilla.org/toolkit/app-startup;1", "nsIAppStartup");
...
var office365_notifier_Controller = {
    ...
    /** @private */
    /** The document of the office 365 web page. */
    _document : undefined,
    /** @private */
    /** The service */
    _service : undefined
...
var office365_notifier_Observer = {
    register : function() {
        Services.obs.addObserver(office365_notifier_Observer, "quit-application", false);
        Services.obs.addObserver(office365_notifier_Observer, "quit-application-granted", false);
        Services.obs.addObserver(office365_notifier_Observer, office365_notifier_Constant.OBSERVER.PREF_SAVED, false);
    },
    unregister : function() {
        Services.obs.removeObserver(office365_notifier_Observer, office365_notifier_Constant.OBSERVER.PREF_SAVED);
        Services.obs.removeObserver(office365_notifier_Observer, "quit-application-granted");
        Services.obs.removeObserver(office365_notifier_Observer, "quit-application");
    },
    observe : function(s, topic, data) {
        if (topic === "quit-application-granted" || topic === "quit-application") {
            ...
            try {
                var srv = office365_notifier_Controller.getService();
                if (srv) {
                    srv.shutdown();
                }
            } finally {
        ...
        } else if (topic === office365_notifier_Constant.OBSERVER.PREF_SAVED) {
            var srv = office365_notifier_Controller.getService();
            if (srv) {
                srv.prefUpdated();
            }
...
office365_notifier_Observer.register();
